...
# =========================================================================
fidelity_arrays = []
for noise_stddev in [0.01, 0.001, 0.0001, 0]:
    # set the random seed
    ...
    def cost(weights, noise_stddev = 0):
        ...
        if noise_stddev:
            random_real = tf.random.normal(shape=target_state.get_shape(), stddev=noise_stddev)
            random_img = tf.random.normal(shape=target_state.get_shape(), stddev=noise_stddev)
            
            current_target_state = target_state + tf.cast(random_real,tf.complex64) + tf.cast(random_img,tf.complex64)*1j


        else:
            current_target_state = target_state
        difference = tf.reduce_sum(tf.abs(ket - current_target_state))
    ...
    for i in range(n_iter):
        ...
        with tf.GradientTape() as tape:
            loss, fid, _, trace = cost(weights,noise_stddev)
            fidelity_array.append(float(fid))
    ...
    cost_after, fidelity_after, ket_after, _ = cost(weights,noise_stddev)
    ...
    fidelity_arrays.append((fidelity_array,noise_stddev))
    
for fidelity_array,noise_stddev in fidelity_arrays:
    plt.plot(np.arange(0,n_iter),fidelity_array,label=f'noise std = {noise_stddev}')
plt.legend()
plt.show()
